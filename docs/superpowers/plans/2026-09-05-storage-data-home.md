...
> **RE-RULED 2026-09-05 (the human): "make it like Langfuse hosted locally."** Read every task below through the master plan's re-rule banner: the data home is **`specboard/data/`** inside the board's folder (never `~/.specboard/`), the fold/runs/report live as rows in **`data/board.db`** (SQLite via `better-sqlite3`, the one new dependency), and only `data/blobs/<sha256>.<ext>` stays file-shaped. Wherever a task body says `~/.specboard/<id>/` or `index.json`, the master's banner is the source.
