...
**Goal:** Every derived file leaves the repository for a content-addressed, gc'd data home (`~/.specboard/<projectId>/`) and ships as **0.45.0**; then the Expected picture of a beat becomes one shared base per screen state plus a patch per moment, built ON that store, and ships as **0.46.0**.
