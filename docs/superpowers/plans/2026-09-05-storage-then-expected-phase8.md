...
> **RE-RULED 2026-09-05 (the human): "make it like Langfuse hosted locally."** This changes WHERE the data home is and WHAT holds the fold, everywhere in this plan and its source plans:
> - **Where:** `specboard/data/` inside the board's own folder (covered by the folder's gitignore) — **not** `~/.specboard/<projectId>/`. The board server is the local host that owns the store, the way a local Langfuse owns its volumes. Every `~/.specboard/<id>/` in the task bodies reads as `specboard/data/` (this repo and demo/todo, which ARE their boards, use `<root>/data/`).
> - **What:** `data/board.db` — a real database, SQLite via `better-sqlite3` (the ONE new dependency; Node 20 lacks `node:sqlite`) — holds everything the plan gave to `index.json`, `runs.json` and `results.json`: the per-screen fold (verdicts, coverage, claims, srcHashes, blob rels), run records, the raw report. `data/blobs/<sha256>.<ext>` is unchanged. Every "index.json" in the task bodies reads as "the fold rows in board.db".
> - **Task deltas:** T1 = the schema + `putBlob` under `specboard/data/` (no `dataHomeRoot` in `~`); T2's keep-set is a db query; T3 imports into db rows; T5/T9/T10 read the db; T7 writes db rows; T8/T11/T12 unchanged in intent (T12 no longer needs the home-dir story); T15 zips `specboard/data/`; A3′–A5′ unchanged except `beats[].base` is a column. `SPECBOARD_HOME` (T15's reviewer flow, test isolation) still works: it overrides the data directory's location.
> - **Team mode (still deferred):** the SAME server hosted on a shared box, Postgres + a bucket behind the same store interface, teammates point `BOARD_URL` at it. The canonical shared record is CI's run, written into that hosted instance.

**Goal:** Every derived file leaves the repository for a content-addressed, gc'd data home (the board's own `specboard/data/`: `board.db` + `blobs/`) and ships as **0.45.0**; then the Expected picture of a beat becomes one shared base per screen state plus a patch per moment, built ON that store, and ships as **0.46.0**.
