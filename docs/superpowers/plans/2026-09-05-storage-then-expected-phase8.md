...
> **PRECEDENCE (the human, 2026-09-06, corrected): where any two documents conflict, the design page `docs/storage-references-design-2026-09-05.html` — the latest — governs the conflicted item; `docs/decisions-c1-c5-2026-09-06.html` records the C1–C5 process rulings (accepted in full, and consistent with the design page); then this banner, which transcribes both; then the task bodies in the source plans. Older pages (`expected-base-patch-plan-2026-09-05.html`, `storage-userflow-2026-09-05.html`) are task-body detail only.**
>
...
> - **C rulings (2026-09-06 — the human accepted every recommendation):** **C1 YES** — optional `{ beat }` argument on `checkReq`, with a lint that refuses a beat number absent from the PRD's behavior block; its own small task in phase 5, before A6′. **C2 BY TEST** — T7 keys evidence by the covering TEST, not the requirement; T3's importer maps the old requirement-key entries onto the new shape; the I5 precedence rule and the `foreign` marker are deleted with the collision they refereed; the reader (A4′) headlines the home screen's own test for a beat row and lists other covering tests beneath. **C3 KEEP** the per-segment names (no work — A4′ preserves the shipped strip). **C4 BLESSED** — R7's original wording stands verbatim as already in the tree (no work). **C5 residue DEFERRED** — the 850 MB history rewrite waits for a human-picked quiet moment after 0.45.0; T11 alone stops the growth.
