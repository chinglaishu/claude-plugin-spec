...
- **Then** each picture carries ONE chip, LED BY THE MOMENT'S OWN WORDS — the label the run recorded, the same on both cells — and under it only what that side holds: `EXPECTED "…"` over the replica, `ACTUAL ✓ "…"` or `ACTUAL ✕ "…"` over the photograph, the mark carrying the state beside the hue — wrapped to read WHOLE inside the cell, with the whole text in a hover tooltip only where a value clamped *(the human, 2026-09-07: "avoid having too much tooltip/pop over" — a tooltip repeating what the chip already shows is noise)*; a moment that claimed nothing carries no chip at all
