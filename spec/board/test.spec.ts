...
test('The detail shows no wireframe or design affordance', async ({ page }) => {
  ...
  await checkReq('R7', async () => {
    ...
    // and nothing EXTERNAL is rendered inside the tool — no wireframe/design iframe. Narrowed
    // 2026-09-03 (rule 4, with the reason attached): this read `iframe` count 0 outright, which the
    // human's Expected View decision made false — a beat row's Expected cell is an
    // `<iframe sandbox srcdoc>` holding the harvest's own committed replica. That is not an
    // artifact this tool owns: it carries no src, loads nothing, runs nothing, and is regenerated by
    // the fold. What R7 forbids is a design or wireframe LOADED from somewhere, so that is what is
    // asserted now — an `<iframe src=…>` anywhere in the detail still fails, and so does a srcdoc
    // frame with any allow-* token, which is how a live component would get back in.
...
const replicaSpecimens = () => {
  ...
  const out: Array<{ rid: string, beat: any, rep: string, exp: string, lay: any }> = []
  for (const rid of Object.keys(ev)) {
    ...
    out.push({ rid, beat, rep, exp, lay: JSON.parse(readFileSync(beat.layoutAfter, 'utf8')) })
  }
...
test('The Expected picture is the app\'s own component — captured, sandboxed, or honestly no picture', async ({ page }) => {
  ...
  await checkReq('R18', async () => {
    ...
    // …AND WHERE THE BEAT MADE A CLAIM, the ringed element says what the requirement ASKED FOR — that
    // is the whole difference between the Expected and the Actual beside it (spec/_replica.mjs applies
    // the claim; a harvest whose checks read no value has none, and then there is nothing to assert).
    const claims = onDisk.claims || []
    const said = await frame.evaluate((f, src) => {
      const d = new DOMParser().parseFromString(String(src), 'text/html')
      const el = d.querySelector('[data-ring]')
      return el ? (el.textContent || '').replace(/\s+/g, ' ').trim() : ''
    }, doc)
    const claimed = claims.filter((c: any) => c && typeof c.expected === 'string' && c.expected.trim())
    if (claimed.length) {
      const last = claimed[claimed.length - 1]
      await hudCheck('the ringed element carries what the requirement asked for', last.expected, said)
      expect(said.includes(String(last.expected).trim()),
        'the Expected\'s ringed element carries the claim\'s expected value: ' + JSON.stringify({ said, want: last.expected })).toBe(true)
    } else {
      // no claim on this harvest — then the picture must still be the app's OWN component: the ringed
      // element is really in it, wearing the box the skeleton measured (never an empty page with a ring
      // floating on it, which is what a lost capture would look like)
      expect(said !== null && /data-ring="/.test(doc),
        'the ringed element is in the picture, not a ring on nothing').toBe(true)
    }
  })
  ...
  await checkReq('R18', async () => {
    ...
    // (b) the harvest moves past the picture → the banner says the LAYOUT moved. Forced onto the baked
    // row the way this file forces every other derived mark (the builder derives the real one by
    // hashing the skeleton on disk against the pin the fold recorded — tools/build-board.mjs evAttrs),
    // then hop-and-back so the reader rebuilds from it.
    const spec = specs[0]
    const hop2 = spec.rid === 'R2' ? 'R3' : 'R2'
    const node = dt.locator(`.reqpane .req[data-r="${spec.rid}"]`)
    await node.evaluate(el => {
      const bs = JSON.parse(el.getAttribute('data-ev-beats') || '[]')
      for (const b of bs) b.lstale = true
    ...
    await node.evaluate(el => {
      const bs = JSON.parse(el.getAttribute('data-ev-beats') || '[]')
      for (const b of bs) delete b.lstale
      el.setAttribute('data-ev-beats', JSON.stringify(bs))
    })
  })
})
