...
const oldG = mk(
  [
    { id: "backend:req-hv-01", type: "requirement", title: "R1" },
    { id: "frontend:hv-1", type: "test", title: "HV-1", status: "pass" },
    { id: "frontend:hv-4", type: "test", title: "HV-4", status: "pass" },
...
const newG = mk(
  [
    { id: "backend:req-hv-01", type: "requirement", title: "R1" },
    { id: "backend:req-hv-02", type: "requirement", title: "R2" },
    { id: "frontend:hv-1", type: "test", title: "HV-1", status: "pass" },
    { id: "frontend:hv-4", type: "test", title: "HV-4", status: "fail" },
  ],
...
describe("computeDelta", () => {
  ...
  it("groups added/removed nodes by type", () => {
    expect(d.added).toEqual([{ type: "requirement", ids: ["backend:req-hv-02"] }]);
    expect(d.removed).toEqual([{ type: "doc", ids: ["backend:gone"] }]);
  });
  it("detects status transitions on shared test ids", () => {
    expect(d.newlyFailing).toEqual(["frontend:hv-4"]);
...
describe("formatDelta", () => {
  it("renders human lines and a first-sync message for null", () => {
    ...
    expect(lines.some((l) => l.includes("newly failing") && l.includes("hv-4"))).toBe(true);
