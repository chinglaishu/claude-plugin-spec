...
const features = [
  feat("frontend:add.mapping", ["**/step2/ColumnMappingStage.test.tsx", "**/svc_ai/**column_mapping**"]),
  feat("frontend:add.validate", ["**/step2/rowValidation.test.ts", "**/svc_ai/validators/**"]),
];
...
describe("matchingFeatures", () => {
  it("returns every feature whose globs match the path (a file can match several = cross-cuts)", () => {
    ...
    expect(
      matchingFeatures("svc_backend/tests/services/svc_ai/validators/test_rent_validator.py", features).sort()
...
describe("deriveUnitTagEdges", () => {
  ...
  it("does not tag e2e tests (they use explicit `features:`), nor untagged unit files", () => {
    const nodes = [
      ...features,
      { id: "frontend:BIL-1", type: "test", kind: "e2e", title: "x", path: "svc_frontend/e2e/cases/x.cases.yaml" } as GraphNode,
