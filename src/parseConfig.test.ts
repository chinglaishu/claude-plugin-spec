...
describe("parseInstruction", () => {
  ...
  it("instruction node + imports edge", () => {
    const { nodes, edges } = parseInstruction({ path: "dojostack_frontend/CLAUDE.md", content: claude });
    expect(nodes[0].type).toBe("instruction");
    expect(nodes[0].id).toBe("frontend:claude");
    expect(nodes[0].title).toBe("claude");
    expect(edges).toContainEqual({ from: nodes[0].id, to: "frontend-instructions", type: "imports", source: "dojostack_frontend/CLAUDE.md" });
  });
  it("ignores non-md @-lines (decorators) and only emits edge for .md imports", () => {
    const content = `# Mixed\n@../x/foo.instructions.md\n@decorator("y")\nsome text`;
    const { nodes, edges } = parseInstruction({ path: "CLAUDE.md", content });
  ...
  it("applies-to from applyTo frontmatter", () => {
    const inst = `---\napplyTo: "dojostack_frontend/**"\n---\nrules`;
    const { nodes, edges } = parseInstruction({ path: ".github/instructions/frontend.instructions.md", content: inst });
    expect(nodes[0].id).toBe("main:frontend-instructions");
    expect(nodes[0].title).toBe("frontend-instructions");
    expect(edges).toContainEqual({ from: nodes[0].id, to: "dojostack_frontend/**", type: "applies-to", source: ".github/instructions/frontend.instructions.md" });
  });
});
...
describe("parseAgent", () => {
  it("agent node from frontmatter", () => {
    ...
    const { nodes } = parseAgent({ path: ".claude/agents/code-review.md", content: a });
...
describe("parseHooks", () => {
  ...
  it("hook node + triggers + activates-on", () => {
    const { nodes, edges } = parseHooks({ path: ".claude/settings.json", content: settings });
