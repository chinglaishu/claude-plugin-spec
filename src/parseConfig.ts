...
export function parseInstruction(input: { path: string; content: string }): ParseResult {
...
export function parseAgent(input: { path: string; content: string }): ParseResult {
...
export function parseHooks(input: { path: string; content: string }): ParseResult {
  const source = input.path;
  ...
  for (const [event, groups] of Object.entries<any>(hooks)) {
    const id = nsId(input.path, `hook:${event}`);
