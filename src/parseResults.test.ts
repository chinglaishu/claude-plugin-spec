...
const graph = {
  generatedAt: "T",
  nodes: [
    { id: "frontend:bil-1", type: "test", title: "BIL-1", status: "todo" },
    { id: "frontend:bil-3", type: "test", title: "BIL-3", status: "todo" },
    { id: "backend:x", type: "doc", title: "X", status: "current" },
  ],
  edges: [], issues: [],
} as any;
const results = JSON.stringify({ generatedAt: "2026-07-02T09:14:00Z", commit: "a1b2c3d", results: { "bil-1": "pass" } });

describe("applyResults", () => {
  it("overrides test status from the results file and stamps lastRun", () => {
    const g = applyResults(graph, results);
    expect(g.nodes.find((n: any) => n.id === "frontend:bil-1").status).toBe("pass");
    expect(g.nodes.find((n: any) => n.id === "frontend:bil-3").status).toBe("todo"); // no result → unchanged
  ...
  const v2results = JSON.stringify({
    ...
    results: { "bil-1": { status: "pass", attempts: 2, at: "2026-07-03T10:00:00Z" } },
  });

  it("v2: stamps status, attempts, runAt and derives flaky for retry-passes", () => {
    const g = applyResults(graph, v2results);
    const n = g.nodes.find((n: any) => n.id === "frontend:bil-1");
  ...
  it("v1 entries never mark flaky (attempts normalize to 1)", () => {
    ...
    expect(g.nodes.find((n: any) => n.id === "frontend:bil-1").flaky).toBeUndefined();
  });
  ...
  const v2errResults = JSON.stringify({
    ...
    results: { "bil-1": { status: "fail", attempts: 2, at: "2026-07-06T08:00:00Z", error: "Error: expect(locator).toBeVisible() failed", commit: "9f8e7d6" } },
  });
  ...
  it("leaves lastError/runCommit absent when the entry predates the fields", () => {
    ...
    const v1n = applyResults(graph, results).nodes.find((n: any) => n.id === "frontend:bil-1");
  ...
  const v2fixResults = JSON.stringify({
    ...
    results: { "bil-1": { status: "fail", attempts: 1, at: "2026-07-07T09:00:00Z",
  ...
  it("carries suggestedFix from the entry onto the node (2b)", () => {
    const n = applyResults(graph, v2fixResults).nodes.find((n: any) => n.id === "frontend:bil-1");
    expect(n.status).toBe("fail");
  ...
  it("leaves suggestedFix absent when the entry has none", () => {
    const n = applyResults(graph, v2errResults).nodes.find((n: any) => n.id === "frontend:bil-1");
