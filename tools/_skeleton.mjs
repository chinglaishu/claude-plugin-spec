...
export const FILES = [
  ...
  // …and the ONE-TIME IMPORTER beside them (2026-09-06): a project updating from a release that
  // committed its harvest runs `node tools/store-import.mjs` once to move that harvest into its data
  // home. It is a CLI entry point nothing else imports, so the every-relative-import-is-vendored
  // guard is blind to it — without this line a project upgrading into the store would have nowhere
  // to run the migration from, and its last fold would simply vanish from the board.
  'tools/store-import.mjs',
...
// What a scaffolded project's git must NOT track, inside spec/ — the transient run state AND every
// derived file (decision A, the human 2026-09-05: "we only store things in codebase if it's
// necessary, otherwise find a way to store somewhere else"). The harvest, the fold, the run log and
// the raw report live in ~/.specboard/<projectId>/ now, so a clone reads proof out of the data home
// rather than out of git; the two pictures a test still writes here (screen.png, crawl.png) are
// regenerated by any run. This REPLACES D2 (the human, 2026-08-22, "the frames are the proof a fresh
// clone must show") — rule 6: the requirement D2 protected is met by location instead, so the
// mechanism changed and the reason is recorded here rather than silently dropped.
//   NOT ignored: spec/_specboard.json (the release, the project id and the two store switches) and
//   spec/_conflict-decisions.json (the human's own rulings — authored, not run state).
//   Still ignored: spec/_config.json — per machine, and its sign-in script may carry a credential.
// Pinned by tools/skeleton-ignore.test.mjs.
export const SPEC_IGNORE = [
  '_state-snapshot.*.json', '_dir-snapshot.*.json', '_run-report.json', '_runs/',
  '_conflicts.json', '_config.json', '_crawl.json', 'crawl.png',
  // the saved authenticated session — real tokens, never committed
  '_auth-state.json',
  // …and everything a run derives (the data home, 2026-09-05/06)
...
// THE STORE'S TWO DRIVERS, as real dependencies of a scaffolded project (the data home, 2026-09-06).
// better-sqlite3 is the default db driver (`board.db` in the data home) and pg is the team's; the
// board SERVER reads through them, not just the test suite, so they are runtime dependencies rather
// than devDependencies. Without them a vendored board throws "the sqlite driver needs better-sqlite3
// (npm install)" the first time anything reads the fold — which is to say, on its first page. Pinned
// exactly (not `^`) and to the versions this repo itself runs: better-sqlite3 is a native module, and
// a project silently resolving a different build than the one specboard is tested against is a class
// of breakage a caret invites. Pinned by tools/sidecar.test.mjs against this repo's own package.json.
export const DEPS = { 'better-sqlite3': '12.11.1', pg: '8.23.0' }
...
// THE RULE (the human, 2026-09-04: "stay everything in a specboard folder in the repo … and apply it
// to any project later on"): the board lives INSIDE the app repo in a folder named `specboard/`. The
// folder name is the convention, so no pointer is needed; `.specboard` stays as the override for a
// board kept anywhere else.
//
// WHAT CHANGED, 2026-09-06 (decision A): the folder is COMMITTED, and only its MACHINERY is ignored —
// inside it, by its own .gitignore (boardIgnoreLines below). The 2026-09-04 shape ignored the whole
// folder wholesale, for one reason: it held the harvest, and the human's instruction was to get the
// images out of the app repo. The harvest is not there any anymore — everything a run derives lives in
// ~/.specboard/<projectId>/, out of every git by location — so what is left in the folder is the
// AUTHORED board (prd.md, test.spec.ts, steps.ts, narration packs, the human's conflict rulings, the
// manifest) plus a byte copy of the plugin. Ignoring that wholesale meant the PRDs and the tests
// existed on exactly one disk, which was the stated trade-off of the local-only step and is what
// committing the folder ends. The scaffold therefore no longer writes `/specboard/` into the app's
// .gitignore; a project that already has that line keeps it until the people who own it decide
// otherwise (an update never edits an app repo's own ignore file).
export const NESTED = 'specboard'

// The board folder's own .gitignore. It hides the folder's MACHINERY — the vendored skeleton (a byte
// copy of the plugin, re-created by scaffold/update, so committing it would be committing the same
// code twice in one repo), the generated board.html, node_modules, the run's scratch and the saved
// session — and NOTHING a person wrote. That difference is the whole reason the folder can be
// committed at all (2026-09-06). Pinned by tools/sidecar.test.mjs and tools/skeleton-ignore.test.mjs.
