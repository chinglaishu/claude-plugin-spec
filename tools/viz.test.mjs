// tools/viz.test.mjs — the WIREFRAME MIRROR kit and the layout pin. Pure units: a harvested layout
// skeleton maps DETERMINISTICALLY to a house-style drawing of the app's own boxes — no model, no fs,
// no browser — and `layoutHash` pins the geometry that drawing (and the replica beside it) was made
// from.
//
// THE SKETCH'S OWN TESTS ARE GONE (2026-09-05, CLAUDE.md rule 4 — a test of a retired export is
// deleted with it). The human retired the archetype SKETCH, so vizHash/vizStale as exports,
// matchArchetype, deriveSchematic, pictureFor, build-board's renderSchematic and the twelve draw*
// templates no longer exist, and every case that pinned them — the hash-pin pair, the archetype
// mapping set, the deriveSchematic contract, the renderSchematic baking contract, the task-7
// archetypes read off the real prds, the task-11 play-speed pass and the pictureFor choice — went
// with them. Nothing they asserted survives elsewhere: an un-harvested requirement now shows its
// Given/When→Then prose and an honest "no Expected yet", which is board R18's own beat.
import { test } from 'node:test'
import assert from 'node:assert/strict'
import { renderWireframe, layoutHash, framedRegion, mirrorGaps, gapSummary } from './viz.mjs'
...
// the demo's REAL committed harvest is read from disk by the mirror-10..13 pins below (this import
...
test('renderWireframe draws the real layout — the asserted value, in the app\'s own words', () => {
  ...
  assert.match(d.svg, /data-viz-hash="[0-9a-f]{16}"/, 'and the TEXT pin, stamped beside the layout one')
})
...
test('no usable layout → null: nothing is drawn from a harvest that measured nothing', () => {
...
test('harvested text is data: escaped, and never able to make the builder refuse the figure', () => {
  ...
  // …and no on*= substring survives the escape, which is the whole claim: an escaped label is DATA,
  // never structure. (Two legs went from here on 2026-09-05, CLAUDE.md rule 4: one asserted that
  // build-board's renderSchematic refuses a wireframe, the other that it still bakes an archetype
  // SKETCH carrying the same hostile text. renderSchematic and deriveSchematic are both retired with
  // the sketch, so both legs asserted functions that no longer exist. The claim itself is proven by
  // the three assertions above, on the drawing this kit still makes.)
  assert.ok(!/\son\w+\s*=/i.test(d.svg), 'no on*= substring survives, wherever it is read')
})
...
// (The pair "STALE BY LAYOUT, NOT ONLY BY TEXT" — that build-board bakes data-viz-layout-stale
// beside data-stale on a drawing's figure — was deleted 2026-09-05 with renderSchematic and the
// SKETCH it baked. The FACT survives on the other picture: tools/build-board.mjs still derives the
// replica's own `lstale` from layoutHash against the harvest on disk, and the storyline's stale
// banner still says "layout moved" — board R18's fourth beat.)
...
// (WHICH PICTURE A REQUIREMENT GETS — the `pictureFor` choice — was deleted 2026-09-05 with the
// SKETCH. Its four cases decided between a wireframe, an archetype sketch and no drawing at all;
// with the sketch retired there is exactly one answer left and no choice to pin: a harvested
// requirement's Expected picture is its replica, and an un-harvested one has none.)
